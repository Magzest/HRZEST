...
function initHeroScanner() {
  ...
  function appendAttendanceLog(name, role, methodLabel, badgeClass, iconClass) {
    ...
    newRow.innerHTML = `
      ...
      <td><span class="badge ${badgeClass}"><i class="fa-solid ${iconClass}"></i> ${methodLabel}</span></td>
      <td>HQ Entrance (Verified)</td>
      <td>${timeNow}</td>
      <td><span style="color:var(--accent-emerald)"><i class="fa-solid fa-circle-check"></i> Verified Just Now</span></td>
    `;
...
function initPayrollCalculator() {
  ...
  function updatePayroll() {
    ...
    if (baseSalaryVal) baseSalaryVal.textContent = `$${base.toLocaleString()}`;
    if (bonusVal) bonusVal.textContent = `$${bonus.toLocaleString()}`;
    if (taxRateVal) taxRateVal.textContent = `${taxRate}%`;

    if (slipGross) slipGross.textContent = `$${base.toLocaleString('en-US', { minimumFractionDigits: 2 })}`;
    if (slipBonus) slipBonus.textContent = `$${bonus.toLocaleString('en-US', { minimumFractionDigits: 2 })}`;
    if (slipTax) slipTax.textContent = `-$${taxDeduction.toLocaleString('en-US', { minimumFractionDigits: 2 })}`;
    if (slipNet) slipNet.textContent = `$${net.toLocaleString('en-US', { minimumFractionDigits: 2 })}`;
  }
...
function initLeaveWorkflow() {
  ...
  approveLeaveBtn?.addEventListener('click', () => {
    if (leaveActionGroup) {
      leaveActionGroup.innerHTML = `<span style="color:var(--accent-emerald); font-weight:700; font-size:0.85rem;"><i class="fa-solid fa-check-circle"></i> Manager Approved</span>`;
    }
    if (leaveDaysCounter) {
...
function initOrgVault() {
  ...
  orgUserBtns.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (emp) {
        if (badgePhoto) badgePhoto.src = emp.photo;
