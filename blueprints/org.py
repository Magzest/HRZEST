...
def api_create_org():
    """JSON API Endpoint to create a new Organisation / Tenant."""
    try:
        ...
        admin_email = data.get("admin_email", "").strip() or None
        submitted_secret = data.get("signup_secret", "").strip()
        plan = data.get("plan", "basic").strip().lower()

        if _SIGNUP_SECRET and submitted_secret:
            if not secrets.compare_digest(_SIGNUP_SECRET, submitted_secret):
                return jsonify({"ok": False, "msg": "Invalid signup secret code."}), 400

        if not company_name or not subdomain or not admin_username or not admin_password:
            return jsonify({
                "ok": False,
                "msg": "Company Name, Subdomain, Admin Username, and Password are required."
            }), 400
