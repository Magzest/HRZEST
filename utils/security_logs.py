...
def send_webhook_alert_async(event_type: str, level: str, message: str, ip: str = "", identifier: str = ""):
    ...
    def _worker():
        ...
        secret = os.environ.get("WEBHOOK_SECRET", "secops_hmac_secret_2026").encode("utf-8")

        if slack_url:
            try:
                slack_body = json.dumps({"text": payload_text}).encode("utf-8")
                sig = hmac.new(secret, slack_body, hashlib.sha256).hexdigest()
                req = urllib.request.Request(
                    slack_url, data=slack_body,
                    headers={"Content-Type": "application/json", "X-SecOps-Signature": f"sha256={sig}"}
                )
        ...
        if teams_url:
            try:
                ...
                sig = hmac.new(secret, teams_body, hashlib.sha256).hexdigest()
                req = urllib.request.Request(
                    teams_url, data=teams_body,
                    headers={"Content-Type": "application/json", "X-SecOps-Signature": f"sha256={sig}"}
                )
                urllib.request.urlopen(req, timeout=5)
            except Exception as e:
